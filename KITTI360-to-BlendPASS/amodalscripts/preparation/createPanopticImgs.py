...
def convert2panoptic(cityscapesPath=None, outputFolder=None, useTrainId=False, setNames=["val", "train", "test"]):
    ...
    if cityscapesPath is None:
        ...
        apspanopath = '../../data/APS_KITTI_360/'

    if outputFolder is None:
        outputFolder = os.path.join(apspanopath, 'Panoptic')
    ...
    for setName in setNames:
        ...
        searchFine   = os.path.join(apspanopath,'Instance', setName, "*_instanceTrainIds.png")
        annotations_json = os.path.join(apspanopath,'annotations', setName+".json")
        anns = cvb.load(annotations_json)
        ...
        outputBaseFile = "apskitti360_panoptic_{}{}".format(setName, trainIfSuffix)
        ...
        for progress, f in enumerate(files):
            ...
            originalSempath = os.path.join(apspanopath, 'Semantic', setName, f.split('/')[-1].replace('_instanceTrainIds.png', '.png'))
            originalSemFormat = np.array(Image.open(originalSempath))

            fileName = os.path.basename(f)

            inputFileName = fileName.replace("_instanceTrainIds.png", ".png")
            imageId = imgs_dict[inputFileName]
            outputFileName = fileName.replace("_instanceTrainIds.png", "_panoptic.png")
            ...
            segmentIds = np.unique(originalSemFormat)
            for segmentId in segmentIds:
                if id2label[segmentId].hasInstances:
                    continue
                if segmentId < 1000:
                    semanticId = segmentId
                    isCrowd = 1
                ...
                mask = originalSemFormat == segmentId
                color = [segmentId % 256, segmentId // 256, segmentId // 256 // 256]
                pan_format[mask] = color
                ...
                segmInfo.append({"id": int(segmentId), "category_id": int(categoryId), "area": int(area), "bbox": bbox,
                                 "iscrowd": isCrowd})
