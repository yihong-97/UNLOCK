...
class SFDADataset(object):
            ...
            for c in self.rcs_classes:
                ...
                for file, pixels in samples_with_class_and_n[c]:
                    if pixels > self.rcs_min_pixels:
                        self.samples_with_class[c].append(file.split('/')[-2] + '/' + file.split('/')[-1])
    ...
    def get_rare_class_sample(self):
        ...
        f1 = f1.replace('.png', '.jpg')
        i1 = self.file_to_idx[f1]
        s2 = self.target[i1]
        if self.rcs_min_crop_ratio > 0:
            for j in range(10):
                n_class = torch.sum(s2['gt_semantic_seg'].data == c)
                ...
                s2 = self.target[i1]
        i2 = np.random.choice(range(len(self.source)))
        # s2 = self.target[i2]
        s1 = self.source[i2]
        # use this below block if you want to visualize the target gt labels and loss weights
        # return {
        #     **s1, # source images, gt labels, loss weights
        #     'target_img_metas': s2['img_metas'],
        #     'target_img': s2['img'],
        #     'target_gt_semantic_seg': s2['gt_semantic_seg'],
        #     'target_gt_center': s2['gt_center'],
        #     'target_center_weights': s2['center_weights'],
        #     'target_gt_offset': s2['gt_offset'],
        #     'target_offset_weights': s2['offset_weights'],
        #     'target_gt_instance_seg': s2['gt_instance_seg'],
        #     'target_gt_foreground_seg': s2['gt_foreground_seg'],
        # }
