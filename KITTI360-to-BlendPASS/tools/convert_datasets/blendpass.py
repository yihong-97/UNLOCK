...
def convert_to_train_id_apskitti(file):
    # re-assign labels to match the format of Cityscapes
    pil_label = Image.open(file)
    label = np.asarray(pil_label)
    sample_class_stats = {}
...
def parse_args():
    parser = argparse.ArgumentParser(
        description='Convert APSKITTI annotations to TrainIds')
    parser.add_argument('--gta_path', default='../../unmaskformer_experiments/exp-00006/240627_2247_Sourceonly_test_best_3e656/oass_eval/soft_labels', help='gta data path')
    parser.add_argument('--gt-dir', default='semantic', type=str)
    parser.add_argument('-o', '--out-dir', default=None, help='output path')
    parser.add_argument(
    ...
    if not only_postprocessing:
        if args.nproc > 1:
            sample_class_stats = mmcv.track_parallel_progress(
                convert_to_train_id_apskitti, poly_files, args.nproc)
        else:
            sample_class_stats = mmcv.track_progress(convert_to_train_id_apskitti,
                                                     poly_files)
    else:
