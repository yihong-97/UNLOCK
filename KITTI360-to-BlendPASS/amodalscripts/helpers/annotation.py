...
from amodalscripts.helpers.labels     import labels, name2label,labelid2name
...
class CsPoly(CsObject):
    ...
    def fromJsonText(self, jsonText, objId=-1):
        self.id = objId
        self.label = str(labelid2name[objId])
        self.polygon = []
        for part in jsonText['i_segm']:
            poly = np.array(part).reshape((int(len(part) / 2), 2))
            self.polygon.append([Point(p[0], p[1]) for p in list(poly)])
...
class Annotation:
    ...
    def fromJsonText(self,jsonDict):
        self.imgWidth = int(jsonDict[0]['width'])
        self.imgHeight = int(jsonDict[0]['height'])
        self.objects = []
        # load objects
        if self.objectType != CsObjectType.IGNORE2D:
            for obj_josn in jsonDict:
                objId = obj_josn['category_id']
                if self.objectType == CsObjectType.POLY:
                    ...
                    obj = CsBbox3d()
                obj.fromJsonText(obj_josn, objId)
    ...
    def fromJsonFile(self, semimg, jsonFile):
        if not os.path.isfile(jsonFile):
            print('Given json file not found: {}'.format(jsonFile))
            return
        with open(jsonFile, 'r') as f:
            jsonText = f.read()
            self.fromJsonText(semimg, jsonText)
