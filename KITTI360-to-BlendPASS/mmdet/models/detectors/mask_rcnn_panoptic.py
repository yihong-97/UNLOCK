...
class MaskRCNNPanoptic(TwoStageDetector):
    ...
    def __init__(self,
        ...
        super(MaskRCNNPanoptic, self).__init__(
                                                ...
                                                neck=neck,
                                                rpn_head=rpn_head,
                                                rpn_head_amodal=rpn_head_amodal,
                                                roi_head=roi_head,
                                                roi_head_amodal=roi_head_amodal,
                                                train_cfg=train_cfg,
                                                test_cfg=test_cfg,
        ...
        self._init_amodal_head(rpn_head_amodal, roi_head_amodal)

    # init amodal instance head 
    def _init_amodal_head(self, rpn_head_amodal, roi_head_amodal):
        """Initialize ``rpn_head_amodal, roi_head_amodal``"""
        if rpn_head_amodal is not None:
            # TODO: whether use different rpn config for amodal ??
            rpn_train_cfg = self.train_cfg.rpn if self.train_cfg is not None else None
            rpn_head_amodal_ = rpn_head_amodal.copy()
            rpn_head_amodal_.update(train_cfg=rpn_train_cfg, test_cfg=self.test_cfg.rpn)
            self.rpn_head_amodal = build_head(rpn_head_amodal_)

        if roi_head_amodal is not None:
            # update train and test cfg here for now
            rcnn_train_cfg = self.train_cfg.rcnn if self.train_cfg is not None else None
            roi_head_amodal.update(train_cfg=rcnn_train_cfg)
            roi_head_amodal.update(test_cfg=self.test_cfg.rcnn)
            roi_head_amodal.pretrained = self.pretrained
            self.roi_head_amodal = build_head(roi_head_amodal)
    ...
    def forward_train(self,
        ...
        if gt_bboxes:
            ...
            for i in range(batch_size):
                if gt_labels[i].numel() != 0:
                    img_metas_bb.append(img_metas[i])
                    gt_bboxes_bb.append(gt_bboxes[i])
                    gt_labels_bb.append(gt_labels[i])
                    gt_masks_bb.append(gt_masks[i])
                    x_n_index.append(i)
            for j in range(len(x_n)):
                x_n_bb.append(x_n[j][x_n_index])
            if len(gt_bboxes_bb) == 0:
                set_loss_to_zero = True
                # break
        ...
        if gt_abboxes:
            batch_size = len(gt_alabels)
            set_loss_to_zero = False
            # for i in range(batch_size):
            #     if gt_alabels[i].numel() == 0:
            #         set_loss_to_zero = True
            #         break
            img_metas_ab = []
            gt_abboxes_ab = []
            ...
            for i in range(batch_size):
                if gt_alabels[i].numel() != 0:
                    img_metas_ab.append(img_metas[i])
                    gt_abboxes_ab.append(gt_abboxes[i])
                    gt_alabels_ab.append(gt_alabels[i])
                    gt_amasks_ab.append(gt_amasks[i])
                    x_n_aindex.append(i)
            for j in range(len(x_n)):
                x_n_ab.append(x_n[j][x_n_aindex])
            if len(gt_abboxes_ab) == 0:
                set_loss_to_zero = True
            if not set_loss_to_zero:
                # RPN forward and loss
                if self.with_rpn:
                    proposal_cfg = self.train_cfg.get('rpn_proposal', self.test_cfg.rpn)
                    rpn_losses_amodal, proposal_list_amodal = self.rpn_head_amodal.forward_train(
                                                                                x_n_ab,
                                                                                img_metas_ab,
                                                                                gt_abboxes_ab,
                                                                                gt_labels=None,
                                                                                gt_bboxes_ignore=gt_abboxes_ignore,
                                                                                proposal_cfg=proposal_cfg,
                                                                                gt_box_domain_indicator=box_domain_indicator,
                                                                                pseudo_wght_val=pseudo_wght_val,
                                                                                use_instance_pseduo_losses=use_instance_pseduo_losses,
                    ...
                    losses.update(add_prefix(rpn_losses_amodal, 'rpn_amodal'))
                else:
                    proposal_list_amodal = proposals
                # RoIHead (includes box and mask head) forward and loss
                roi_losses_amodal = self.roi_head_amodal.forward_train(
                                                                x_n_ab,
                                                                img_metas_ab,
                                                                proposal_list_amodal,
                                                                gt_abboxes_ab,
                                                                gt_alabels_ab,
                                                                gt_abboxes_ignore,
                                                                gt_amasks_ab,
                ...
                losses.update(add_prefix(roi_losses_amodal, 'roi_amodal'))
        return losses
    ...
    def simple_test(self, img, img_meta, rescale=True, proposals=None):
        ...
        output = list(output)
        # results['sem_probabilitys'] = output
        # maskrcnn stuff below:
        assert self.with_bbox, 'Bbox head must be implemented.'
        if proposals is None:
            proposal_list = self.rpn_head.simple_test_rpn(x_n, img_meta)
            proposal_list_amodal = self.rpn_head_amodal.simple_test_rpn(x_n, img_meta)
        else:
            proposal_list = proposals
            proposal_list_amodal = proposals
        inst_pred = self.roi_head.simple_test(x_n, proposal_list, img_meta, rescale=rescale)
        inst_pred_amodal = self.roi_head_amodal.simple_test(x_n, proposal_list_amodal, img_meta, rescale=rescale)
        results['ins_results'] = inst_pred
        results['ins_results_amodal'] = inst_pred_amodal
        return [results]
...
@DETECTORS.register_module()
class MaskRCNNPanoptic_PL_ALL(TwoStageDetector):
    ...
    def __init__(self,
                 ...
                 use_neck_feat_for_decode_head=False,
                 auxiliary_head=None):
        super(MaskRCNNPanoptic_PL_ALL, self).__init__(
                                                backbone=backbone,
                                                neck=neck,
                                                rpn_head=rpn_head,
                                                rpn_head_amodal=rpn_head_amodal,
