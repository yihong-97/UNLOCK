...
def get_dataset_base_dacs(include_diffusion_data, source, target, evalScale):
    if not include_diffusion_data:
        if evalScale:
            dacs_dataset_base = f'_base_/datasets/uda_{source}_to_{target}_maskrcnn_oass_evalScale_{evalScale}.py'
        else:
            dacs_dataset_base = f'_base_/datasets/uda_{source}_to_{target}_maskrcnn_oass.py'
    else:
        dacs_dataset_base = f'_base_/datasets/uda_{source}_to_{target}_maskrcnn_oass_diffusion.py'
...
def get_dataset_base(uda, source, target, include_diffusion_data=False, evalScale=None):
    ...
    return {
            'target-only': f'_base_/datasets/{uda}_{target}_maskrcnn_oass.py',
            'source-only': f'_base_/datasets/{uda}_{source}_to_{target}_maskrcnn_oass.py',
