...
def strong_transform_target(param, data=None, target=None):
    assert ((data is not None) or (target is not None))
    ...
    data, target = color_jitter(
        ...
        data=data,
        target=None)
    data, target = gaussian_blur(blur=param['blur'], data=data, target=None)
    return data, target
...
def get_class_masks_dummy(labels):
    class_masks = []
    for label in labels:
        classes = torch.unique(labels)
        nclasses = 0 # classes.shape[0]
        class_choice = np.random.choice(nclasses, int((nclasses + nclasses % 2) / 2), replace=False)
        classes = classes[torch.Tensor(class_choice).long()]
        class_masks.append(generate_class_mask(label, classes).unsqueeze(0))
    return class_masks

def get_class_masks_only_thing(labels):
    class_masks = []
    for label in labels:
        classes = torch.unique(labels)
        classes_thing = classes[classes >= 11]
        classes_thing = classes_thing[classes_thing <= 18]
        if classes_thing.numel() == 0:
            classes_thing = classes
        nclasses = classes_thing.shape[0]
        class_choice = np.random.choice(nclasses, int((nclasses + nclasses % 2) / 2), replace=False)
        classes_thing = classes_thing[torch.Tensor(class_choice).long()]
        class_masks.append(generate_class_mask(label, classes_thing).unsqueeze(0))
    return class_masks
