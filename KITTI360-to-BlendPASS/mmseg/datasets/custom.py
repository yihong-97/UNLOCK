...
from mmseg.utils.visualize_onlypred import save_onlypredictions, save_all_onlypredictions
...
from blendpassscripts.evaluation.evalAmodalPanopticSemanticLabeling import print_cpu_memory_usage
...
class CustomDataset(Dataset):
    ...
    def load_blendpass_annotations_amodal(self, pano_dir, ann_file):
        ...
        if 'train' in self.img_dir:
            ...
            for img in img_list:
                img_info = {}
                img_info['filename'] = img.split('/')[-2] + '/' +img.split('/')[-1]
                if self.pse_lab_dir is None:
                    img_info['ann'] = {}
                else:
                    img_info['ann'] = {}
                    img_info['ann']['semlabel_name'] = os.path.join(self.pse_lab_dir, 'semantic')
                    img_info['ann']['inslabel_name'] = os.path.join(self.pse_lab_dir, 'instance')
                    img_info['ann']['ainslabel_name'] = os.path.join(self.pse_lab_dir, 'amodal_instance')
                img_infos.append(img_info)
                # i += 1
                # if i > 8:
                #     break
    ...
    def get_gt_panoptic_labels(self, device, logger, debug,
        ...
        for img_info in self.img_infos:
            ...
            if labels_for_maskformer:
                ...
                # gt_panoptic_labels.append([data['gt_semantic_seg'], data['gt_masks'], data['gt_labels'], data['gt_bboxes'], data['gt_amasks'], data['gt_abboxes']])
            else:
    ...
    def evaluate_oass_for_maskrcnn(self, results, device=None, panop_eval_temp_folder=None,
        ...
        '''
        print_cpu_memory_usage('begin evaluation process', logger)
        ...
        if dataset_name == 'cityscapes':
            ...
            panoptic_josn_file = 'val_trainId_K2B.json'
            panoptic_json_folder = 'val_trainId_K2B'
            stuff_area = 2048
        ...
        # print_cpu_memory_usage('begin evaluation process try', logger)
        ...
        try:
            for i in range(num_samples):
                ...
                if dataset_name == 'cityscapes':
                    ...
                    raise NotImplementedError(f'Implementation not found for dataset: {dataset_name}')
                # print_cpu_memory_usage('process results {}, image: {}'.format(i,image_filename_list[i]), logger)
                ...
                if evaluate_from_saved_numpy_predictions:
                    ...
                    # instance seg
                    results_i = np.load(results[i], allow_pickle=True)[0]
                    out_dict['boxes'] = results_i['ins_results'][0][0]
                    out_dict['masks'] = results_i['ins_results'][0][1]
                    # instance seg
                    out_dict['aboxes'] = results_i['ins_results_amodal'][0][0]
                    out_dict['amasks'] = results_i['ins_results_amodal'][0][1]
                    # semantic seg
                    out_dict['semantic'] = results_i['sem_results'][0]
                ...
                if 'mIoU' in metric and 'mAP' not in metric and 'mPQ' not in metric and evalScale == '1024x200':
                    semantic_prediction = torch.from_numpy(results_i['sem_results'][0]).float().to(device)
                ...
                ## TODO: evaluate the mPQ of amodal panoptic?
                ...
                if visuals_pan_eval or visuals_all_eval:
                    ...
                    TrgInsPd = ins_seg.cpu().numpy()
                    TrgAmoInsPd = amodal_seg
                    for a_i, ap_p in enumerate(TrgAmoInsPd):
                        TrgAmoInsPd[a_i] = TrgAmoInsPd[a_i].squeeze(0).cpu().numpy()
                    ...
                    if visuals_all_eval:
                        ...
                        save_all_predictions(dataset_name, image_filename, gt_labels, TrgSemGT, TrgSemPd, TrgInsPd, TrgPanPd,
                                             TrgAmoInsPd, TrgAmoPanPd, outdir, debug, self.img_dir, self.pano_dir, resize_with_pad)

        except Exception:
            logger.exception("Exception during testing:")
            raise
        finally:
            del gt_panoptic_labels
            del results
            print_cpu_memory_usage('begin evaluation', logger)
            eval_results = {}
            ...
            if 'mAPQ' in metric:
                ...
                del amodal_panoptic_results
                del amodal_panoptic_metric
    ...
    def generate_labels_oass_for_maskrcnn(self, results_path_list, device=None, panop_eval_temp_folder=None, dataset_name=None,
                                   gt_dir=None, debug=False, num_samples_debug=None, gt_dir_insta=None,
                                   gt_dir_panop=None, gt_dir_aomodal=None, logger=None, post_proccess_params=None,
                                   visuals_pan_eval=None, visuals_all_eval=None, out_dir=None, metric='mIoU',
                                   evalScale=None, evaluate_from_saved_numpy_predictions=None, ):
        ...
        intersec_th = post_proccess_params['intersec_th']
        upsnet_mask_pruning = post_proccess_params['upsnet_mask_pruning']
        generate_thing_cls_panoptic_from_instance_pred = post_proccess_params[
            'generate_thing_cls_panoptic_from_instance_pred']

        # The below params are to evaluate the M-Net, i.e., you first train two networks for semseg and instseg.
        # Once training is done, we fuse the predictions of these two networks at inference time.
        # to generate the panoptic segmentation prediction maps, and evaluate the predictions to get a mPQ for M-Net .
        # (c.f. Please refer to our main paper for M-Net)
        DUMP_OASS_pred_labels_as_numpy_array = post_proccess_params['dump_oass_pred_labels_as_numpy_array']
        DELETE_OASS_ALL_SAVE_NUMPY_RESULTS = post_proccess_params['delete_oass_all_save_numpy_results']
        if DUMP_OASS_pred_labels_as_numpy_array:
            DUMP_OASSPre_Save_Path = os.path.join(panop_eval_temp_folder, 'pred_numpys')
            DUMP_SemanticPre_Save_Path = os.path.join(DUMP_OASSPre_Save_Path, 'semantic')
            os.makedirs(DUMP_SemanticPre_Save_Path, exist_ok=True)
            DUMP_InstancePre_Save_Path = os.path.join(DUMP_OASSPre_Save_Path, 'instance')
            os.makedirs(DUMP_InstancePre_Save_Path, exist_ok=True)
            DUMP_AInstancePre_Save_Path = os.path.join(DUMP_OASSPre_Save_Path, 'amodal_instance')
            os.makedirs(DUMP_AInstancePre_Save_Path, exist_ok=True)
            DUMP_OASSLab_Save_Path = os.path.join(panop_eval_temp_folder, 'lab_numpys')
            DUMP_SemanticLab_Save_Path = os.path.join(DUMP_OASSLab_Save_Path, 'semantic')
            os.makedirs(DUMP_SemanticLab_Save_Path, exist_ok=True)
            DUMP_InstanceLab_Save_Path = os.path.join(DUMP_OASSLab_Save_Path, 'instance')
            os.makedirs(DUMP_InstanceLab_Save_Path, exist_ok=True)
            DUMP_AInstanceLab_Save_Path = os.path.join(DUMP_OASSLab_Save_Path, 'amodal_instance')
            os.makedirs(DUMP_AInstanceLab_Save_Path, exist_ok=True)
        ...
        # get all the GT panoptic labels for all images in the val set
        # gt_panoptic_labels, new_img_shapes = self.get_gt_amodal_labels(device, logger, debug,
        #     labels_for_maskformer=True, dataset_name=dataset_name, evalScale=evalScale)
        ...
        #     numpys_path = os.path.join(strs[0], strs[1], strs[2], strs[3], strs[4], 'results_numpys')
        #     npy_file_list = os.listdir(numpys_path)
        #     assert len(gt_panoptic_labels) == len(
        #         npy_file_list), 'The number of gt labels and predictions are not the same !!'
        #     pass
        # else:
        #     assert len(gt_panoptic_labels) == len(
        #         results), 'The number of gt labels and predictions are not the same !!'
        ...
        image_filename_list = []
        for i in range(len(self.img_infos)):
            image_filename_list.append(self.img_infos[i]['filename'])
        ...
        if dataset_name == 'cityscapes':
            ...
            panoptic_josn_file = 'val_trainId.json'
            panoptic_json_folder = 'val_trainId'
            stuff_area = 2048
            input_image_size = (1024, 200) if debug else (2048, 400)  # Not in use
        else:
            raise NotImplementedError(f'Implementation not found for dataset: {dataset_name}')
        ...
        try:
            for i in range(num_samples):
                if dataset_name == 'cityscapes':
                    ...
                    image_filename = image_filename_list[i].split('.')[0]
                elif dataset_name == 'mapillary':
                    image_filename = image_filename_list[i]
                else:
                    raise NotImplementedError(f'Implementation not found for dataset: {dataset_name}')
                ...
                if DUMP_OASS_pred_labels_as_numpy_array:
                    cityname = image_filename_list[i].split('/')[0]
                    Prenpsave_file_path_ss = os.path.join(DUMP_SemanticPre_Save_Path, cityname)
                    os.makedirs(Prenpsave_file_path_ss, exist_ok=True)
                    Prenpsave_file_path_ins = os.path.join(DUMP_InstancePre_Save_Path, cityname)
                    os.makedirs(Prenpsave_file_path_ins, exist_ok=True)
                    Prenpsave_file_path_ains = os.path.join(DUMP_AInstancePre_Save_Path, cityname)
                    os.makedirs(Prenpsave_file_path_ains, exist_ok=True)
                    cityname = image_filename_list[i].split('/')[0]
                    Labnpsave_file_path_ss = os.path.join(DUMP_SemanticLab_Save_Path, cityname)
                    os.makedirs(Labnpsave_file_path_ss, exist_ok=True)
                    Labnpsave_file_path_ins = os.path.join(DUMP_InstanceLab_Save_Path, cityname)
                    os.makedirs(Labnpsave_file_path_ins, exist_ok=True)
                    Labnpsave_file_path_ains = os.path.join(DUMP_AInstanceLab_Save_Path, cityname)
                    os.makedirs(Labnpsave_file_path_ains, exist_ok=True)
                start_time = time.time()
                out_dict = {}
                # gt_labels = {}

                results = np.load(results_path_list[i], allow_pickle=True).tolist()[0]
                out_dict['boxes'] = results['ins_results'][0][0]
                out_dict['masks'] = results['ins_results'][0][1]
                # instance seg
                out_dict['aboxes'] = results['ins_results_amodal'][0][0]
                out_dict['amasks'] = results['ins_results_amodal'][0][1]
                # semantic seg
                out_dict['semantic'] = results['sem_results'][0]
                out_dict['semantic_pro'] = results['sem_probabilitys'][0]
                if DELETE_OASS_ALL_SAVE_NUMPY_RESULTS:
                    strCmd = 'rm' + ' ' + results_path_list[i]
                    logger.info('Removing the intermediate results: {}'.format(results_path_list[i]))
                    os.system(strCmd)
                ...
                if 'mIoU' in metric and 'mAP' not in metric and 'mPQ' not in metric and evalScale == '1024x200':
                    ...
                    semantic_prediction = semantic_prediction.unsqueeze(dim=0)
                    semantic_prediction = semantic_prediction.unsqueeze(dim=0)
                    semantic_prediction = F.interpolate(semantic_prediction, size=(200, 1024), mode='bilinear',
                                                        align_corners=False)
                    semantic_prediction = semantic_prediction.long()
                    semantic_prediction = semantic_prediction.squeeze(dim=0)
                    semantic_prediction = semantic_prediction.squeeze(dim=0)
                ...
                if DUMP_OASS_pred_labels_as_numpy_array:
                    npsave_file_path = os.path.join(DUMP_SemanticLab_Save_Path, f'{image_filename}.npy')
                    np.save(npsave_file_path, out_dict['semantic'])
                    logger.info(f'Semantic predictions saved at: {npsave_file_path}')
                    npsave_file_path = os.path.join(DUMP_SemanticPre_Save_Path, f'{image_filename}.npy')
                    np.save(npsave_file_path, out_dict['semantic_pro'])
                    logger.info(f'Semantic predictions saved at: {npsave_file_path}')
                # instance_metric update
                if 'mAP' in metric:
                    if upsnet_mask_pruning:
                        instances, ins_seg, pan_seg_thing = get_cityscapes_instance_format_for_maskrcnn_v3(
                            out_dict['boxes'], out_dict['masks'], pred_shape=pred_shape, mask_score_th=mask_th,
                            sem_seg=out_dict['semantic'], device=device, thing_list=thing_list,
                            use_semantic_decoder_for_instance_labeling=use_semantic_decoder_for_instance_labeling,
                            use_semantic_decoder_for_panoptic_labeling=use_semantic_decoder_for_panoptic_labeling,
                            nms_th=nms_th, intersec_th=intersec_th, )
                    else:
                        instances, ins_seg, pan_seg_thing = get_blendpass_instance_format_for_maskrcnn(
                            out_dict['boxes'], out_dict['masks'], pred_shape=pred_shape, mask_score_th=mask_th,
                            sem_seg=out_dict['semantic'], device=device, thing_list=thing_list,
                            ...
                            nms_th=nms_th, intersec_th=intersec_th, )
                    if DUMP_OASS_pred_labels_as_numpy_array:
                        npsave_file_path = os.path.join(DUMP_InstancePre_Save_Path, f'{image_filename}.npy')
                        np.save(npsave_file_path, instances)
                        logger.info(f'Instance predictions saved at: {npsave_file_path}')
                    # instance_metric.update(instances, image_filename, debug=False, logger=logger)

                if 'mAAP' in metric:
                    if upsnet_mask_pruning:
                        amodals, amodal_seg, amodal_seg_thing = get_cityscapes_instance_format_for_maskrcnn_v3(
                            out_dict['aboxes'], out_dict['amasks'], pred_shape=pred_shape, mask_score_th=amask_th,
                            sem_seg=out_dict['semantic'], device=device, thing_list=thing_list,
                            use_semantic_decoder_for_instance_labeling=use_semantic_decoder_for_instance_labeling,
                            use_semantic_decoder_for_panoptic_labeling=use_semantic_decoder_for_panoptic_labeling,
                            nms_th=nms_th, intersec_th=intersec_th, )
                        ...
                        amodals, amodal_seg, amodal_seg_thing = get_blendpass_amodal_format_for_maskrcnn(
                            ...
                            nms_th=nms_th, intersec_th=intersec_th, )
                    if DUMP_OASS_pred_labels_as_numpy_array:
                        npsave_file_path = os.path.join(DUMP_AInstancePre_Save_Path, f'{image_filename}.npy')
                        np.save(npsave_file_path, amodals)
                        logger.info(f'AmodalInstance predictions saved at: {npsave_file_path}')
                    # amodal_instance_metric.update(amodals, gt_labels['amodal'], image_filename, debug=False,
                    #                               logger=logger)
                ...
                if 'mPQ' in metric:
                    ...
                    if generate_thing_cls_panoptic_from_instance_pred and pan_seg_thing is not None:
                        pan_seg_thing = torch.from_numpy(pan_seg_thing).long().to(device)
                        panoptic_pred = merge_semantic_and_instance_v2(out_dict['semantic'].unsqueeze(dim=0),
                                                                       # [1, 512, 1024]
                                                                       pan_seg_thing.unsqueeze(dim=0),  # [1, 512, 1024]
                                                                       label_divisor, thing_list, stuff_area,
                                                                       void_label=label_divisor * ignore_label)  # 255000
                    else:
                        panoptic_pred = merge_semantic_and_instance(out_dict['semantic'].unsqueeze(dim=0),
                                                                    # [1, 512, 1024]
                                                                    ins_seg.unsqueeze(dim=0),  # [1, 512, 1024]
                                                                    label_divisor, thing_list, stuff_area,
                                                                    void_label=label_divisor * ignore_label)  # 255000
                    ...
                    if 'mPQ' in metric:
                        panoptic_pred = panoptic_pred.squeeze(0).cpu().numpy()
                        if DUMP_OASS_pred_labels_as_numpy_array:
                            npsave_file_path = os.path.join(DUMP_InstanceLab_Save_Path, f'{image_filename}.npy')
                            np.save(npsave_file_path, panoptic_pred)
                            logger.info(f'Instance_after_merge predictions saved at: {npsave_file_path}')
                ...
                if 'mAPQ' in metric or 'mAAP' in metric:
                    ...
                    for a_i, ap_p in enumerate(amodalpanoptic_pred):
                        amodalpanoptic_pred[a_i] = amodalpanoptic_pred[a_i].squeeze(0).cpu().numpy()
                    if DUMP_OASS_pred_labels_as_numpy_array:
                        npsave_file_path = os.path.join(DUMP_AInstanceLab_Save_Path, f'{image_filename}.npy')
                        np.save(npsave_file_path, amodalpanoptic_pred[1:])
                        logger.info(f'AmoddalInstance_after_merge predictions saved at: {npsave_file_path}')
                    # if 'mAPQ' in metric:
                    #     amodal_panoptic_metric.update(amodalpanoptic_pred, gt_labels['semantic'], gt_labels['amodal'],
                    #                                   image_filename=image_filename, image_id=image_id, debug=debug,
                    #                                   logger=logger)
                # Logging
                torch.cuda.synchronize(device)
                post_time.update(time.time() - start_time)
                ...
                if visuals_pan_eval or visuals_all_eval:
                    ...
                    outdir = eval_folder['visuals']
                    if visuals_pan_eval:
                        save_onlypredictions(dataset_name, image_filename, TrgSemPd, TrgPanPd,
                                         TrgAmoPanPd, outdir, debug, self.img_dir, self.pano_dir, resize_with_pad)
                    if visuals_all_eval:
                        save_all_onlypredictions(dataset_name, image_filename, TrgSemPd, TrgPanPd,
                                             TrgAmoPanPd, outdir, debug, self.img_dir, self.pano_dir, resize_with_pad)
