...
# Cityscapes imports
from blendpassscripts.helpers.csHelpers import printError
# from blendpassscripts.helpers.labels import labels as csLabels
...
OFFSET = 256 * 256 * 256
VOID = 255
amodal_pan_VOID = 255000

import psutil

def print_cpu_memory_usage(pre_cont,logger):
    # 获取CPU使用率
    cpu_usage = psutil.cpu_percent()

    # 获取内存使用情况
    memory_usage = psutil.virtual_memory()
    logger.info("In {}--> CPU:{}, Memory:{}.".format(pre_cont, cpu_usage, memory_usage.percent))
...
def pq_compute_single_core(proc_id, annotation_set, gt_folder, pred_folder, categories,
    ...
    for gt_ann, pred_ann in annotation_set:
        # if idx % 30 == 0:
        # logger.info('Core: {}, {} from {} images processed, image is {}'.format(proc_id, idx, len(annotation_set), gt_ann['file_name']))
        # idx += 1
        ...
        if 'mapillary' in dataset_name:
            ...
            # pan_gt = Image.open(os.path.join(gt_folder,  gt_ann['file_name']))
            # if debug:
            #     pan_gt = pan_gt.resize((1024, 200), Image.NEAREST)
            # pan_gt = np.array(pan_gt, dtype=np.uint32)
            # # pan_gt = np.array(Image.open(os.path.join(gt_folder, gt_ann['file_name'])), dtype=np.uint32) # original
            # pan_gt = rgb2id(pan_gt)

            pan_gts = []
            for gt_img_name in gt_ann['save_file_names']:
                pan_gt = Image.open(os.path.join(gt_folder, gt_img_name))
                if debug:
                    pan_gt = pan_gt.resize((1024, 200), Image.NEAREST)
                pan_gt = np.array(pan_gt, dtype=np.uint32)
                pan_gt = rgb2id(pan_gt)
                pan_gts.append(pan_gt)
        else:
            NotImplementedError('no implementation found at blendpassscripts/evaluation/evalPanopticSemanticLabeling.py --> def pq_compute_single_core(...)')

        # pan_pred = np.array(Image.open(os.path.join(pred_folder, pred_ann['file_name'])), dtype=np.uint32)
        # pan_pred = rgb2id(pan_pred)

        pan_preds = []
        for gt_img_name in pred_ann['save_file_names']:
            pan_pred = np.array(Image.open(os.path.join(pred_folder, gt_img_name)), dtype=np.uint32)
            pan_pred = rgb2id(pan_pred)
            pan_preds.append(pan_pred)
        ...
        # print_cpu_memory_usage('predicted segments area', logger)
        pred_labels_set = set(el['id'] for el in pred_ann['segments_info'])
        for pan_pred in pan_preds:
            labels, labels_cnt = np.unique(pan_pred, return_counts=True)
            for label, label_cnt in zip(labels, labels_cnt):
                if label not in pred_segms:
                    if label == VOID:
                        continue
                    if label == amodal_pan_VOID:
                        continue
        ...
        # print_cpu_memory_usage('confusion matrix calculation', logger)
        gt_pred_map = {}
        # for pan_gt, pan_pred in zip(pan_gts, pan_preds):
        #     pan_gt_pred = pan_gt.astype(np.uint64) * OFFSET + pan_pred.astype(np.uint64)
        #     labels, labels_cnt = np.unique(pan_gt_pred, return_counts=True)
        #     for label, intersection in zip(labels, labels_cnt):
        #         gt_id = label // OFFSET
        #         pred_id = label % OFFSET
        #         if gt_id == amodal_pan_VOID or pred_id ==amodal_pan_VOID:
        #             continue
        #         gt_pred_map[(gt_id, pred_id)] = intersection


        for pan_gt in pan_gts:
            for pan_pred in pan_preds:
                pan_gt_pred = pan_gt.astype(np.uint64) * OFFSET + pan_pred.astype(np.uint64)
                labels, labels_cnt = np.unique(pan_gt_pred, return_counts=True)
                for label, intersection in zip(labels, labels_cnt):
        ...
        # print_cpu_memory_usage('count all matched pairs', logger)
        gt_matched = set()
        pred_matched = set()
...
def print_results(results, categories, logger):
    ...
    logger.info("#" * 41)
    logger.info("- Amodal Panoptic")
    logger.info("-" * 41)
    logger.info("{:14s}| {:>5s}  {:>5s}  {:>5s}".format("Category", "APQ", "ASQ", "ARQ"))
    ...
    logger.info("-" * 41)
    logger.info("{:14s}| {:>5s}  {:>5s}  {:>5s} {:>5s}".format("", "APQ", "SAQ", "ARQ", "N"))

    for name in metrics:
        logger.info("{:14s}| {:5.2f}  {:5.2f}  {:5.2f} {:5d}".format(
            name,
            100 * results[name]['pq'],
            100 * results[name]['sq'],
            100 * results[name]['rq'],
