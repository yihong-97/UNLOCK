...
def build_train_model(cfg, train_cfg=None, test_cfg=None):
    ...
    assert cfg.model.get('train_cfg') is None or train_cfg is None, 'train_cfg specified in both outer field and model field '
    assert cfg.model.get('test_cfg') is None or test_cfg is None, 'test_cfg specified in both outer field and model field '
    if 'uda' in cfg:
        ...
        cfg.sfda['model'] = cfg.model
        cfg.sfda['max_iters'] = cfg.runner.max_iters
        cfg.sfda['log_level'] = cfg.log_level
        return SFDA.build(cfg.sfda, default_args=dict(train_cfg=train_cfg, test_cfg=test_cfg))
    else:
        return DETECTORS.build(cfg.model, default_args=dict(train_cfg=train_cfg, test_cfg=test_cfg))
