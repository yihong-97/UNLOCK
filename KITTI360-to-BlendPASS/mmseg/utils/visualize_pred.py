...
from blendpassscripts.helpers.labels import id2label
...
def prep_gt_pan_for_vis(pan_map, dataset_name='cityscapes', debug=False, blend_ratio=0.7, img=None, runner_mode='train', ax=None, label_divisor=1000,):
    thing_classes = [11,12,13,14,15,16,17]
...
def prep_gt_amopan_for_vis(pan_map, gtlabels, shape, dataset_name='cityscapes', debug=False, blend_ratio=0.7, img=None, runner_mode='train', ax=None, label_divisor=1000,):
    thing_classes = [11,12,13,14,15,16,17]
    colormap = create_label_colormap()
    colored_label = np.zeros((shape[0], shape[1], 3), dtype=np.uint8)
    taken_colors = set([0, 0, 0])
    polygons = []
    for lab in np.unique(gtlabels['semantic']):
        if lab == 255:
            continue
        if lab < 1000:
            classId = lab
        ...
        labelInfo = id2label[classId]
        classTrainId = labelInfo.trainId
        ## attention: Avoid being obscured in the front
        # if classTrainId in thing_classes:
        #     continue
        mask = gtlabels['semantic']
        if mask.shape != shape:
        ...
        if tuple(base_color) not in taken_colors:
            ...
            while True:
                ...
                if color not in taken_colors:
                    taken_colors.add(color)
                    break
        colored_label[mask_bool] = color


    for a_i, lab in enumerate(gtlabels['amodal']['labels']):
        if lab == 255:
            continue
        if lab < 1000:
            classId = thing_classes[lab]
        else:
            classId = lab // label_divisor

        labelInfo = id2label[classId]
        classTrainId = labelInfo.trainId
        mask = gtlabels['amodal']['amasks'].masks[a_i]
        if mask.shape != shape:
            mask = cv2.resize(mask, (shape[1],shape[0]), interpolation=cv2.INTER_NEAREST)
        mask_bool = mask != 0
        ## attention: Avoid being obscured in the front
...
def prep_ins_for_vis(ins_map, dataset_name='cityscapes', debug=False,
                     blend_ratio=0.7, img=None, runner_mode='train',
                     ax=None, label_divisor=None, gen_cvrn_panop_visuals=False):
    ...
    if not gen_cvrn_panop_visuals:
        ...
        thing_classes = [10, 11, 12, 13, 14, 15]
        colormap = create_label_colormap_16cls()
    colored_label = np.zeros((ins_map.shape[0], ins_map.shape[1], 3), dtype=np.uint8)
    mask_label = np.zeros((ins_map.shape[0], ins_map.shape[1]), dtype=np.uint8)
    taken_colors = set([0, 0, 0])
    polygons = []
    for lab in np.unique(ins_map):
        if label_divisor is None:
            labDiv = 10000 if mapId2Domain(lab) == 1 else 1000
        else:
            labDiv = label_divisor
    ...
    if img is not None:
        colored_label = blend_ratio * colored_label + (1 - blend_ratio) * img
        colored_label[mask_label==0] = img[mask_label==0]
    p = PatchCollection(polygons, facecolor='none', edgecolors='w', linewidths=2, alpha=1.0)
    ax.add_collection(p)
    return colored_label.astype(int)
def prep_amopan_for_vis(pan_map, amopan_map, shape, dataset_name='cityscapes', debug=False,
                     blend_ratio=0.7, img=None, runner_mode='train',
                     ax=None, label_divisor=None, gen_cvrn_panop_visuals=False):
    ...
    taken_colors = set([0, 0, 0])
    polygons = []
    for lab in np.unique(amopan_map[0]):
        if label_divisor is None:
            labDiv = 10000 if mapId2Domain(lab) == 1 else 1000
        else:
            labDiv = label_divisor
...
def prep_amoins_for_vis(pan_map, amoins_map, shape, dataset_name='cityscapes', debug=False,
                     blend_ratio=0.7, img=None, runner_mode='train',
                     ax=None, label_divisor=None, gen_cvrn_panop_visuals=False):
    ...
    for lab_mask in amoins_map:
        ...
        mask_bool = mask == lab
        classTrainId = int(lab // labDiv)
        classTrainId = thing_classes[classTrainId]
        if classTrainId == 255 and gen_cvrn_panop_visuals:
            continue
        if classTrainId == 0:
            continue
...
def prep_predins_for_vis(img, masks, labels,
                     blend_ratio=0.3,
                     ax=None, label_divisor=None, gen_cvrn_panop_visuals=False):
    with torch.no_grad():
    ...
    if not gen_cvrn_panop_visuals:
        ...
        thing_classes = [10, 11, 12, 13, 14, 15]
        colormap = create_label_colormap_16cls()
    colored_label = np.zeros((img.shape[0], img.shape[1], 3), dtype=np.uint8)
    mask_label = np.zeros((img.shape[0], img.shape[1]), dtype=np.uint8)
    taken_colors = set([0, 0, 0])
    polygons = []
    for i in range(len(labels)):
        # colored_label_i = np.zeros((img.shape[0], img.shape[1], 3), dtype=np.uint8)
        label = thing_classes[labels[i]]
        mask = (masks[i].masks).reshape(img.shape[0], img.shape[1]) == 1

        mask_label[mask] = 1
        if label == 255 and gen_cvrn_panop_visuals:
            continue
        base_color = colormap[label]
        if tuple(base_color) not in taken_colors:
            taken_colors.add(tuple(base_color))
            color = base_color
        else:
            while True:
                ...
                if color not in taken_colors:
                    taken_colors.add(color)
                    break
        # colored_label[(masks[i].masks).reshape(img.shape[0], img.shape[1]) == 255] = [255, 255, 255]
        colored_label[mask] = color
        # colored_label_i[mask] = color
        # colored_label_i[(masks[i].masks).reshape(img.shape[0], img.shape[1]) == 255] = [255, 255, 255]
        # colored_label_i = blend_ratio * colored_label_i + (1 - blend_ratio) * img * 255
        #
        # cv2.imwrite(os.path.join('/home/yihong/data2/cyh2/project/2405_SFDA_Amodal/UnmaskFormer_SFDA/unmaskformer_experiments/debug', str(np.random.randint(100))+'.png'), colored_label_i[:,:,::-1])
        ...
        polygons += [Polygon(c) for c in contours]
    if len(labels):
        img[(masks[0].masks).reshape(img.shape[0], img.shape[1]) == 255] = [0, 0, 0]
    if img is not None:
        colored_label = blend_ratio * colored_label + (1 - blend_ratio) * img * 255
        # colored_label[mask_label==0] = img[mask_label==0]# * 255
...
def save_all_predictions_bottomup(TrgSemGT,
                            TrgCntGT,
                            TrgOfsGT,
                            TrgInstGT,
                            out_dir,
                            ...
                            dataset_name,
                            logger
                            , img_dir, ann_dir, resize_with_pad):  # TrgInstPd
    ...
    if debug and dataset_name == 'cityscapes':
        ...
        vis_W = 2048
        vis_H = 1024
    elif debug and dataset_name == 'blendpass':
        vis_W = 1024
        vis_H = 200
    elif not debug and dataset_name == 'blendpass':
        vis_W = 2048
        vis_H = 400
    elif dataset_name == 'mapillary':
        raise NotImplementedError(f'Implementation not found for {dataset_name} dataset !!')

    input_img_name = image_filename.replace('_panoptic', '')
    input_img_name = input_img_name + '.png'
    input_img_path = os.path.join(img_dir,  input_img_name)
    # output_img_path = image_filename.split('/')[0]
    # output_img_path = os.path.join(out_dir, output_img_path)
    os.makedirs(out_dir, exist_ok=True)
    output_filename = image_filename.replace('_panoptic', '_predictions') + '.png'
    output_img_path = os.path.join(out_dir, output_filename)
    output_sem_filename = os.path.join(out_dir, 'semantic', f'{output_filename[:-4]}_sem.png')
    output_ins_filename = os.path.join(out_dir, 'instance', f'{output_filename[:-4]}_ins.png')
    output_ains_filename = os.path.join(out_dir, 'amodal_instance', f'{output_filename[:-4]}_ains.png')
    output_pan_filename = os.path.join(out_dir, 'panoptic', f'{output_filename[:-4]}_pan.png')
    output_apan_filename = os.path.join(out_dir, 'amodal_panoptic', f'{output_filename[:-4]}_apan.png')
    ...
    vis_trg_img = np.asarray(vis_trg_img, np.int_)
    rows, cols = 4, 2
    blend_ratio = 0.5
    fig, axs = plt.subplots(rows, cols, figsize=(48, 24), constrained_layout=True)
    ...
    subplotimgV2(axs[1][0], prep_sem_for_vis(TrgSemPd, mode='PD', dataset_name=dataset_name, debug=debug, blend_ratio=blend_ratio, img=vis_trg_img), 'TrgSemPd_img')
    subplotimgV2(axs[1][1], prep_sem_for_vis(TrgSemPd, mode='PD', dataset_name=dataset_name, debug=debug, blend_ratio=1.0, img=None), 'TrgSemPd')
    subplotimgV2(axs[2][0], prep_pan_for_vis(TrgPanPd, dataset_name=dataset_name, debug=debug, blend_ratio=blend_ratio, img=vis_trg_img, runner_mode='val', ax=axs[2][0], label_divisor=1000), 'TrgPanPd_img')
    ...
    for ax in axs.flat:
        ax.set_title('')
    save_subfig(fig, axs[1][0], output_sem_filename.replace('.png', '_img.png'))
    save_subfig(fig, axs[1][1], output_sem_filename)
    save_subfig(fig, axs[2][0], output_pan_filename.replace('.png', '_img.png'))
    save_subfig(fig, axs[2][1], output_pan_filename)
    plt.close()
...
def save_predictions(dataset_name, image_filename, gt_labes, TrgSemGT, TrgSemPd, TrgPanPd, TrgAmoPanPd, outdir, debug, img_dir, ann_dir, resize_with_pad):
    if dataset_name == 'cityscapes':
        vis_W = 1024
        vis_H = 512
        fext = '.png'
        ...
        vis_H = 200
        fext = '.png'
        filename1 = image_filename.replace('_panoptic', '')
        filename2 = filename1 + '.png'
        filename3 = image_filename + '.png'
        output_filename = os.path.join(outdir, f'{filename1}_with_GT.png')
        input_filename = os.path.join(img_dir, filename2)
    elif dataset_name == 'mapillary':
        vis_W = 1024
        vis_H = 768
    ...
    subplotimgV2(axs[3][0], prep_gt_amopan_for_vis(TrgPanGT, gt_labes, TrgPanPd.shape, dataset_name=dataset_name, debug=debug,
                                                blend_ratio=1.0, img=None, runner_mode='val',
                                                ax=axs[3][0], label_divisor=1000), 'TrgAmoPanGT')
    subplotimgV2(axs[3][1], prep_amopan_for_vis(TrgPanPd, TrgAmoPanPd, TrgPanPd.shape, dataset_name=dataset_name, debug=debug,
                                             blend_ratio=1.0, img=None, runner_mode='val',
                                             ax=axs[3][1], label_divisor=1000), 'TrgPanPd')
    for ax in axs.flat:
        ax.axis('off')
...
# def save_all_predictions(dataset_name, image_filename, gt_labes, TrgSemGT, TrgSemPd, TrgPanPd, TrgAmoPanPd, outdir, debug, img_dir, ann_dir, resize_with_pad):
#     if dataset_name == 'cityscapes':
#         vis_W = 1024
#         vis_H = 512
#         fext = '.png'
...
#         filename2 = filename1 + '.png'
#         filename3 = image_filename + '.png'
#         output_filename = os.path.join(outdir, f'{filename1}_Pred_all.png')
#         output_sem_filename = os.path.join(outdir, 'semantic', f'{filename1}_sem.png')
#         output_ins_filename = os.path.join(outdir, 'instance', f'{filename1}_ins.png')
#         output_ains_filename = os.path.join(outdir, 'amodal_instance', f'{filename1}_ains.png')
#         output_pan_filename = os.path.join(outdir, 'panoptic', f'{filename1}_pan.png')
#         output_apan_filename = os.path.join(outdir, 'amodal_panoptic', f'{filename1}_apan.png')
#         input_filename = os.path.join(img_dir, filename2)
#     elif dataset_name == 'mapillary':
#         vis_W = 1024
#         vis_H = 768
...
#     TrgPanGT = rgb2id(TrgPanGT)
#     rows, cols = 4, 2
#     fig, axs = plt.subplots(rows, cols, figsize=(48, 24))
#     blend_ratio = 0.5
#     subplotimgV2(axs[0][0], vis_trg_img, 'TrgImg')
#     subplotimgV2(axs[1][0], prep_sem_for_vis(TrgSemPd, mode='PD', dataset_name=dataset_name,
#                                              debug=debug, blend_ratio=blend_ratio, img=vis_trg_img), 'TrgSemPD_img')
#
#     subplotimgV2(axs[1][1], prep_sem_for_vis(TrgSemPd, mode='PD', dataset_name=dataset_name, debug=debug,
#                                              blend_ratio=1.0, img=None), 'TrgSemPd')
#
#     subplotimgV2(axs[2][0], prep_pan_for_vis(TrgPanPd, dataset_name=dataset_name, debug=debug,
#                                                 blend_ratio=blend_ratio, img=vis_trg_img, runner_mode='val',
#                                                 ax=axs[2][0], label_divisor=1000), 'TrgPanPd_img')
#
#     subplotimgV2(axs[2][1], prep_pan_for_vis(TrgPanPd, dataset_name=dataset_name, debug=debug,
#                                              blend_ratio=1.0, img=None, runner_mode='val',
#                                              ax=axs[2][1], label_divisor=1000), 'TrgPanPd')
#
#     subplotimgV2(axs[3][0], prep_amopan_for_vis(TrgPanPd, TrgAmoPanPd, TrgPanPd.shape, dataset_name=dataset_name, debug=debug,
#                                                 blend_ratio=blend_ratio, img=vis_trg_img, runner_mode='val',
#                                                 ax=axs[3][0], label_divisor=1000), 'TrgAmoPanPD_img')
#
#     subplotimgV2(axs[3][1], prep_amopan_for_vis(TrgPanPd, TrgAmoPanPd, TrgPanPd.shape, dataset_name=dataset_name, debug=debug,
#                                              blend_ratio=1.0, img=None, runner_mode='val',
#                                              ax=axs[3][1], label_divisor=1000), 'TrgAmoPanPD')
#
#     for ax in axs.flat:
#         ax.axis('off')
#     plt.savefig(output_filename)
...
#     save_subfig(fig, axs[2][0], output_pan_filename.replace('.png', '_img.png'))
#     save_subfig(fig, axs[2][1], output_pan_filename)
#     save_subfig(fig, axs[3][0], output_apan_filename.replace('.png', '_img.png'))
#     save_subfig(fig, axs[3][1], output_apan_filename)
#
#     plt.close()


def save_all_predictions(dataset_name, image_filename, gt_labes, TrgSemGT, TrgSemPd, TrgInsPd, TrgPanPd, TrgAmoInsPd, TrgAmoPanPd, outdir, debug, img_dir, ann_dir, resize_with_pad):
    if dataset_name == 'cityscapes':
        vis_W = 1024
        vis_H = 512
        fext = '.png'
    ...
    subplotimgV2(axs[4][0], prep_ins_for_vis(TrgInsPd, dataset_name=dataset_name, debug=debug, blend_ratio=blend_ratio,
                                             img=vis_trg_img, runner_mode='val', ax=axs[4][0], label_divisor=1000),
                 'TrgInsPd_img')

    subplotimgV2(axs[4][1],
                 prep_ins_for_vis(TrgInsPd, dataset_name=dataset_name, debug=debug, blend_ratio=1.0, img=None,
                                  runner_mode='val', ax=axs[4][1], label_divisor=1000), 'TrgInsPd')

    subplotimgV2(axs[5][0],
                 prep_amoins_for_vis(TrgPanPd, TrgAmoInsPd, TrgPanPd.shape, dataset_name=dataset_name, debug=debug,
                                     blend_ratio=blend_ratio, img=vis_trg_img, runner_mode='val', ax=axs[5][0],
                                     label_divisor=1000), 'TrgAmoPanPD_img')

    subplotimgV2(axs[5][1],
                 prep_amoins_for_vis(TrgPanPd, TrgAmoInsPd, TrgPanPd.shape, dataset_name=dataset_name, debug=debug,
                                     blend_ratio=1.0, img=None, runner_mode='val', ax=axs[5][1], label_divisor=1000),
                 'TrgAmoPanPD')
    ...
    save_subfig(fig, axs[3][0], output_apan_filename.replace('.png', '_img.png'))
    save_subfig(fig, axs[3][1], output_apan_filename)
    save_subfig(fig, axs[4][0], output_ins_filename.replace('.png', '_img.png'))
    save_subfig(fig, axs[4][1], output_ins_filename)
    save_subfig(fig, axs[5][0], output_ains_filename.replace('.png', '_img.png'))
    save_subfig(fig, axs[5][1], output_ains_filename)
