...
def get_blendpass_instance_format_for_maskrcnn(
                                                    boxes,
                                                    masks,
                                                    pred_shape=(1024, 2048),
                                                    mask_score_th=0.0,
    ...
    thing_list_mapids = {0: 11, 1: 12, 2: 13, 3: 14, 4: 15, 5: 16, 6: 17}
    assert len(boxes) == len(masks), 'boxes and masks lists must have same length!'
    num_classes = len(boxes)
    instances = []
    ins_seg = np.zeros(pred_shape).astype(int)
    ...
    for c in range(num_classes):
        ...
        if boxes_c.shape[0] > 0:
            ...
            for m in range(len(masks_c)):
                ...
                class_id_tracker[class_id] += 1
                if mask_score >= mask_score_th:
                    ins_seg[masks_c[m]] = class_id * 1000 + new_ins_id
                    ins_cnt += 1
    return instances, ins_seg, None


def get_blendpass_amodal_format_for_maskrcnn(
                                                    aboxes,
                                                    amasks,
                                                    pred_shape=(1024, 2048),
                                                    mask_score_th=0.0,
    ...
    thing_list_mapids = {0: 11, 1: 12, 2: 13, 3: 14, 4: 15, 5: 16, 6: 17}
    assert len(aboxes) == len(amasks), 'boxes and masks lists must have same length!'
    num_classes = len(aboxes)
    instances_amodal = []
    ins_seg_amodal = []
    ins_cnt = 1
    for c in range(num_classes):
        ...
        if boxes_c.shape[0] > 0:
            ...
            for m in range(len(masks_c)):
                ...
                ins['pred_mask'] = np.array(masks_c[m], dtype='uint8')
                ins['score'] = mask_score
                instances_amodal.append(ins)
                if mask_score >= mask_score_th:
                    amodal_seg = masks_c[m] * (c*1000 + ins_cnt)
                    ins_seg_amodal.append(amodal_seg)
                    ins_cnt += 1
    return instances_amodal, ins_seg_amodal, None
