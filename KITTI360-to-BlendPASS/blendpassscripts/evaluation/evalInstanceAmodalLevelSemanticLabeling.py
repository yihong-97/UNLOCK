...
from blendpassscripts.evaluation.amodal import *
import numpy as np
...
def getPrediction( groundTruthFile , args ):
    ...
    csFile = groundTruthFile.split('/')[-1][:-len('_gt.txt')]
    filePattern = "{}_pred.txt".format(csFile)
...
args.gtAmodalFile    = os.path.join(os.path.dirname(os.path.realpath(__file__)),'gtAmodals.json')
...
args.gtAmodalsPath = None
args.predictionWalk = None
args.gtInstancePath = None
...
def readGTImage(gtImageFileName, args, mapillary_dataloading_style='OURS',
    ...
    if 'mapillary' in dataset_name:
        ...
        if debug:
            logger.info('in debug mode downsample the BlendPASS GT labels to 512 x 1024 : From def readGTImage() --> blendpassscripts/evaluation/evalAmodalLevelSemanticLabeling.py')
    ...
    return img
def amodals2dict(imageFileList, verbose=False, dataset_name=None, rgb2id=None, input_image_size=None, mapillary_dataloading_style='OURS', debug=False):
    imgCount     = 0
    amodalDict = {}
    ...
    for imageFileName in imageFileList:
        ...
        if (not os.path.isfile(imageFileName)):
            printError("Infofile '{}' for the ground truth not found.".format(imageFileName))
        amodalInfo = {}
        with open(imageFileName, 'r') as f:
            for line in f:
                ...
                if len(splittedLine) != 3:
                    printError(
                        "Invalid ground truth file. Expected content: relPathGroundTruth1 labelIDGroundTruth1 amodalidtoinstanceid")
                if os.path.isabs(splittedLine[0]):
                    printError("Invalid ground truth file. First entry in each line must be a relative path.")

                filename = os.path.join(os.path.dirname(imageFileName), splittedLine[0])
                filename = os.path.abspath(filename)

                # check if that file is actually somewhere within the prediction root
                if os.path.commonprefix([filename, args.gtAmodalsPath]) != args.gtAmodalsPath:
                    printError("Predicted mask {} in prediction text file {} points outside of prediction path.".format(
                        filename, imageFileName))

                imageInfo = {}
                imageInfo["labelID"] = int(float(splittedLine[1]))
                imageInfo["amodaltoinstance"] = splittedLine[2]
                amodalInfo[filename] = imageInfo
        # # Load image
        ...
        #     imgNp = np.array(img)
        # else:
        #     NotImplementedError('no implementation found at def instances2dict(...) --> blendpassscripts/evaluation/instances2dict.py')
        ...
        for gtImageFile in amodalInfo:
            labelID = amodalInfo[gtImageFile]["labelID"]
            instanceID = amodalInfo[gtImageFile]["amodaltoinstance"]

            gtImage = Image.open(gtImageFile)
            gtNp = np.array(gtImage)
            amodalObj = Amodal(gtImageFile, gtNp, labelID, instanceID)
            amodals[id2label[amodalObj.labelID].name].append(amodalObj.toDict())

        imgKey = os.path.abspath(imageFileName)
        amodalDict[imgKey] = amodals
        imgCount += 1
...
def getGtAmodals(
    ...
    if (os.path.isfile(args.gtAmodalFile)): # False
        if not args.quiet:
            logger.info("Loading ground truth instances from JSON.")
        with open(args.gtAmodalFile) as json_file:
            gtAmodals = json.load(json_file)
    # otherwise create it

    else:
        if (not args.quiet):
            logger.info("Creating ground truth instances from png files.")
        gtAmodals = amodals2dict(
        ...
        writeDict2JSON(gtAmodals, args.gtAmodalFile)
...
def matchGtWithPreds(predictionList, groundTruthList, gtInstances, args, mapillary_dataloading_style='OURS',
    ...
    for (pred, gt) in zip(predictionList, groundTruthList):
        ...
        if 'cityscapes' in dataset_name:
            ...
            gt_base = gt_base[:-len('_gt')]
        assert gt_base in pd_base, 'GT image file name must match with the predicted image file name'

        # key for dicts
        dictKey = os.path.abspath(gt)
        instanceImgFile = os.path.join(args.gtInstancePath, gt_base.replace('panoptic', 'instanceTrainIds.png'))
        # Read input files
        gtInstImage  = readGTImage(
                                    instanceImgFile,
                                    args,
        ...
        (curGtInstances,curPredInstances) = assignGt2Preds(curGtInstancesOrig, predInfo, gtInstImage, args)
...
def assignGt2Preds(gtAmodalsOrig, predInfo, gtInstImage, args):
    ...
    gtAmodals = deepcopy(gtAmodalsOrig)
    for label in gtAmodals:
        for gt in gtAmodals[label]:
            gt["matchedPred"] = []

    # Make the gt a numpy array
    gtInstNp = np.array(gtInstImage)
    ...
    boolVoid = np.in1d(gtInstNp, voidLabelIDList).reshape(gtInstNp.shape)
    ...
    for predImageFile in predInfo:
        ...
        predAmodal = {}
        predAmodal["imgName"]          = predImageFile
        predAmodal["predID"]           = predAmodalCount
        predAmodal["labelID"]          = int(labelID)
        predAmodal["pixelCount"]       = predPixelCount
        predAmodal["confidence"]       = predConf
        predAmodal["voidIntersection"] = np.count_nonzero(np.logical_and(boolVoid, boolPredInst))
        ...
        for (gtNum,gtAmodal) in enumerate(gtAmodalsOrig[labelName]):

            gtImage = Image.open(gtAmodal["gtImageFile"])
            gtNp = np.array(gtImage)
            intersection = np.count_nonzero( np.logical_and( gtNp!=0 , boolPredInst) )

            # If they intersect add them as matches to both dicts
            if (intersection > 0):
                gtCopy   = gtAmodal.copy()
                predCopy = predAmodal.copy()
                ...
                gtAmodals[labelName][gtNum]["matchedPred"].append(predCopy)

        predAmodal["matchedGt"] = matchedGt
        predAmodalCount += 1
        predAmodals[labelName].append(predAmodal)

    return (gtAmodals,predAmodals)
...
def printResults(avgDict, args, logger):
    ...
    if not args.csv:
        logger.info("#"*lineLen)
        logger.info("- Amodal Instance")
        logger.info("-" * lineLen)
    line  = bold
    line += "{:<15}".format("what"      ) + sep + col1
    line += "{:>15}".format("AAP"        ) + sep
    line += "{:>15}".format("AAP_50%"    ) + sep
    if args.distanceAvailable:
        line += "{:>15}".format("AAP_50m"    ) + sep
        line += "{:>15}".format("AAP_100m"   ) + sep
        line += "{:>15}".format("AAP_50%50m" ) + sep
    line += noCol
    logger.info(line)
    if not args.csv:
        logger.info("-"*lineLen)
...
def evaluateImgLists(
    ...
    if debug:
        list_keys = gtAmodals.keys()
        for k in list_keys:
            logger.info(f'gtInstances key : {k}')

    # print_cpu_memory_usage('match predictions and ground truth', logger)
    if True:
    ...
    # evaluate matches
    # print_cpu_memory_usage('evaluate matches', logger)
...
def main():
    ...
    print("Note that this tool uses the file '{}' to cache the ground truth instances.".format(args.gtAmodalFile))
