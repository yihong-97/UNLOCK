...
from mmdet.apis import multi_gpu_test, single_gpu_test_uda, single_gpu_test_uda_for_visual_debug, single_gpu_test_uda_dump_results_to_disk, single_gpu_test_uda_save
...
def main(args):
    ...
    panop_eval_folder = cfg['evaluation']['panop_eval_folder']
    logger.info(f'Evaluation results will be saved at: {panop_eval_folder}')
    ...
    if not distributed:
        ...
        if cfg.generate_only_visuals_without_eval:
            outputs = single_gpu_test_uda_for_visual_debug(
                ...
                panop_eval_temp_folder=panop_eval_folder,
                )
        elif cfg.dump_predictions_to_disk:
            dump_path = os.path.join(cfg.work_dir, 'results_numpys')
            os.makedirs(dump_path,exist_ok=True)
            outputs_path_list = single_gpu_test_uda_dump_results_to_disk(
                model,
                data_loader,
                out_dir=dump_path,
                debug=cfg.debug,
            ...
            outputs, out_dir_inter = single_gpu_test_uda_save(
                model,
                data_loader,
                show=args.show,
    ...
    if cfg.generate_only_visuals_without_eval:
        ...
        cfg['evaluation']['panop_eval_folder'] = cfg['panop_eval_folder_previous']
    ...
    elif cfg.dump_predictions_to_disk and outputs_path_list:
        eval_kwargs = cfg.get('evaluation', {}).copy()
        for key in ['interval', 'tmpdir', 'start', 'gpu_collect', 'save_best', 'rule', 'dynamic_intervals']:
            eval_kwargs.pop(key, None)
        dataset.evaluate(outputs_path_list, logger=logger, **eval_kwargs)
        # logger.info(metric)
        # metric_dict = dict(config=args.config, metric=metric)
        # if json_file is not None and rank == 0:
        #     mmcv.dump(metric_dict, json_file)
        ...
        if outputs:
            ...
            if rank == 0:
                ...
                if json_file  is not None and rank == 0:
                    mmcv.dump(metric_dict, json_file)
            if out_dir_inter:
                strCmd = 'rm -r' + ' ' + out_dir_inter
                os.system(strCmd)
                print(f'executing : {strCmd}')
