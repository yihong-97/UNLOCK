...
def single_gpu_test_uda_dump_results_to_disk(model,
    ...
    for i, data in enumerate(data_loader):
        with torch.no_grad():
            ...
            if dataset_name == 'blendpass' and 'train' in img_file_path:
                img_filename = data['img_metas'][0].data[0][0]['ori_filename']
                save_name_path1 = os.path.join(out_dir, img_filename.split('/')[0])
                os.makedirs(save_name_path1, exist_ok=True)
            else:
        ...
        #     prog_bar.update()
        results_path_list.append(npy_fname)
        prog_bar.update()
        # if i >= 50:
        #     break
    return results_path_list
...
def single_gpu_test_uda_for_visual_debug(model,
    ...
    for i, data in enumerate(data_loader):
        if True:
            ...
            out_dir = os.path.join(eval_folder['visuals'], 'unmaskformer_pred_visuals', cityname)
...
def single_gpu_test_uda_save(model,
                    data_loader,
                    show=False,
                    out_dir=None,
                    debug=False,
    ...
    PALETTE = getattr(dataset, 'PALETTE', None)
    prog_bar = mmcv.ProgressBar(len(dataset))
    out_dir_inter = osp.join(out_dir, 'infer_temp')
    os.makedirs(out_dir_inter, exist_ok=True)
    for i, data in enumerate(data_loader):
        with torch.no_grad():
            if debug or dataset_name == 'mapillary':
                rescale = False
            ...
            if dataset_name == 'blendpass' and 'train' in img_file_path:
                ...
                img_filename = img_file_path.split('/')[-1]
                img_format = img_filename.split('.')[-1]
            npy_fname = os.path.join(out_dir_inter, img_filename.replace(img_format, 'npy'))
            np.save(npy_fname, result)
        batch_size = len(result)
        results.append(npy_fname)
        for _ in range(batch_size):
            prog_bar.update()
    return results, out_dir_inter
