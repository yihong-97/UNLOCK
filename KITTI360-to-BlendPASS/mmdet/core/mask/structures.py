...
class BitmapMasks(BaseInstanceMasks):
    ...
    def crop_and_resize(self,
        ...
        if num_bbox > 0:
            ...
            print((gt_masks_th==255).sum().item())
    ...
    def crop_and_resize_with_ignore(self,
                        bboxes,
                        out_shape,
                        inds,
                        device='cpu',
        ...
        if num_bbox > 0:
            masks = (self.masks == 1)
            masks_ignore = (self.masks == 255)
            gt_masks_th = torch.from_numpy(masks).to(device).index_select(
                0, inds).to(dtype=rois.dtype)
            # print((gt_masks_th==1).sum().item())
            targets = roi_align(gt_masks_th[:, None, :, :], rois, out_shape,
                                1.0, 0, 'avg', True).squeeze(1)
            gt_masks_ign_th = torch.from_numpy(masks_ignore).to(device).index_select(
                0, inds).to(dtype=rois.dtype)
            # print((gt_masks_ign_th==1).sum().item())
            targets_ign = roi_align(gt_masks_ign_th[:, None, :, :], rois, out_shape,
                                1.0, 0, 'avg', True).squeeze(1)
            if binarize:
                resized_masks = (targets >= 0.5).cpu().numpy()
            else:
                # print((targets == 1).sum().item())
                # print((targets_ign == 1).sum().item())
                targets = (targets >= 0.5).cpu().numpy()
                targets[targets_ign.cpu().numpy()==1] = 255
                resized_masks = targets#.cpu().numpy()
                # print((resized_masks == 255).sum().item())
                # print((resized_masks == 1).sum().item())
        else:
            resized_masks = []
        return BitmapMasks(resized_masks, *out_shape)
    def expand(self, expanded_h, expanded_w, top, left):
