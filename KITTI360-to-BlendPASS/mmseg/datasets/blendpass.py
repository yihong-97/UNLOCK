    ...
    def evaluate(self, results, metric='mIoU', logger=None, imgfile_prefix=None, efficient_test=False,
        ...
        if len(metrics) > 0:
            if eval_type == 'daformer':
                ...
                eval_results.update(
                    super(BlendPASS, self).evaluate_panoptic(results, cuda, panop_eval_temp_folder, dataset_name,
            ...
            elif eval_type == 'maskrcnn_oass_pl':
                eval_results.update(
                    super(BlendPASS, self).generate_labels_oass_for_maskrcnn(
                        results, cuda, panop_eval_temp_folder, dataset_name, gt_dir, debug, num_samples_debug,
                        gt_dir_insta, gt_dir_panop, gt_dir_amodal, logger, post_proccess_params, visuals_pan_eval, visuals_all_eval, out_dir, metric, evalScale
                                                                                )
                )
